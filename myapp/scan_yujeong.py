...
# 바코드를 스캔하고 OCR API를 통해 정보를 추출하는 함수
...
# JSON 파일에서 제품 정보를 검색하는 함수
def search_product_in_json(barcode):
    try:
        with open(json_file_path, mode="r", encoding="utf-8") as file:
            barcode_data = json.load(file)
        for item in barcode_data:
            if item.get("BAR_CD") == barcode:
                return item
        return None
